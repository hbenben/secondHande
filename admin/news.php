...
?>
   ...
   <div class="title_right"><strong>公告管理</strong></div>
   <?php
   $sql=mysql_query("select count(*) as total from news",$conn);
...
<?php
		   ...
		   if ($total<=$pagesize){
			...
			if(($page)==""){
             ...
             $sql1=mysql_query("select * from news order by id desc limit ".($page-1)*$pagesize.",$pagesize ",$conn);
   ...
   <tr align="middle" bgcolor="#FFFFFF" onMouseOut="mOut(this,'#FFFFFF');" onMouseOver="mOvr(this,'#DCF9B9');" style="cursor: default;">
    ...
    <td align="center"><a href="newsedit.php?id=<?=$info1['id']?>">修改</a>&nbsp;<a href="?id=<?=$info1['id']?>&action=del" onClick="return confirm('确定要删除吗？')">删除</a></TD>
    </tr>
<?php
	}
?>     
